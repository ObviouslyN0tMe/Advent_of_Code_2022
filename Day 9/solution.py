# puzzle input
with open("testinput") as file:
    rawdata = [x.strip("\n") for x in file.readlines()]


class Coordinate:
    def __init__(self, x: int, y: int):
        self.x = x
        self.y = y

    def __add__(self, other):
        return Coordinate(self.x + other.x, self.y + other.y)

    def __sub__(self, other):
        return Coordinate(self.x - other.x, self.y - other.y)


def adjustTail(head, tail):
    diff_x = head.x - tail.x
    diff_y = head.y - tail.y
    if abs(diff_x) == 2:
        return Coordinate(head.x + diff_x/2, head.y)
    elif abs(diff_y) == 2:
        return Coordinate(head.y, head.y + diff_y/2)
    else:
        print(tail.x, tail.y)
        return tail


movement_dict = {"L": Coordinate(-1, 0), "R": Coordinate(1, 0), "U": Coordinate(0, 1), "D": Coordinate(0, -1)}

head_coord = Coordinate(0, 0)
tail_coord = Coordinate(0, 0)

tail_positions = {(0, 0): True}

for line in rawdata:
    move = line.split(" ")
    for i in range(int(move[1])):
        head_coord += movement_dict[move[0]]
        tail_coord = adjustTail(head_coord, tail_coord)
        tail_positions[(tail_coord.x, tail_coord.y)] = True

print(tail_positions)
print(len(tail_positions))
